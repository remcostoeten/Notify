...
export async function runRelease(
        ...
        if (!options.skipConfirm) {
            ...
            if (!proceed) {
                return {
                    ...
                    errors: ['Aborted: uncommitted changes']
                }
            }
        }
    } else {
    ...
    if (options.version) {
        ...
        const bumped = semver.inc(config.version, options.bump, undefined, options.preid)
        if (!bumped) {
            return {
                success: false,
                version: config.version,
                errors: [`Invalid version bump: ${options.bump}`]
            }
        }
        newVersion = bumped
    } else {
        ...
        const { bump } = await prompt<{ bump: VersionBump }>({
            ...
            choices: [
                {
                    name: 'patch',
                    message: `patch  ${config.version} → ${semver.inc(config.version, 'patch')}`
                },
                {
                    name: 'minor',
                    message: `minor  ${config.version} → ${semver.inc(config.version, 'minor')}`
                },
                {
                    name: 'major',
                    message: `major  ${config.version} → ${semver.inc(config.version, 'major')}`
                },
                {
                    name: 'prerelease',
        ...
        newVersion = semver.inc(config.version, bump, undefined, options.preid)!
    }
