...
export async function loadConfig(cwd: string = process.cwd()): Promise<Partial<ReleaseConfig>> {
    ...
    const pkgConfig = (packageJson['release-cli'] ?? packageJson['release']) as
        | Partial<ReleaseConfig>
        | undefined
...
export function loadLLMConfig(existing?: Partial<LLMConfig>): LLMConfig {
    const provider =
        existing?.provider ?? (process.env['LLM_PROVIDER'] as LLMProvider) ?? detectLLMProvider()
